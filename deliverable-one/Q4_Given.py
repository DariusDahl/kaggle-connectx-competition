...
def my_agent(obs, config):
    ...
    for col in valid_moves:
        column = obs.board[col::config.columns]
        filled = config.rows - column.count(0)
        if min_filled is None or filled < min_filled:
            min_filled = filled
            min_col = [col]
        elif filled == min_filled:
            min_col.append(col)
    return random.choice(min_col)
